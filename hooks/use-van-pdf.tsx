...
export default function useVanPdf({
    ...
    const criteriaDescriptions = useMemo(
        () => ({
            ...
            aphasia:
                "Trouble naming objects, repeating sentences, or following commands (not just slurred speech)",
            neglect:
                "Ignoring one side, eyes/head turned to one side, or not feeling one side",
        }),
        []
    );
        ...
        try {
            ...
            if (isAssessmentComplete) {
                if (isVANPositive) {
                    ...
                    doc.text(
                        "Result: VAN Positive - High Likelihood of LVO",
                        20,
                        58
                    );
                } else {
            ...
            if (weaknessPresent === null) {
                ...
                doc.setTextColor(
                    weaknessPresent ? 200 : 34,
                    weaknessPresent ? 100 : 197,
                    weaknessPresent ? 0 : 94
                );
                doc.text(
                    `Status: ${weaknessPresent ? "Present" : "Not Present"}`,
                    30,
                    87
                );
                ...
                if (!weaknessPresent) {
                    doc.setTextColor(100, 100, 100);
                    doc.text(
                        "Note: Without weakness, VAN is negative.",
                        30,
                        94
                    );
                }
            ...
            if (weaknessPresent === true) {
                ...
                const tableData = [
                    ...
                    [
                        "Vision",
                        checkItems.vision ? "Present" : "Absent",
                        criteriaDescriptions.vision,
                    ],
                    [
                        "Aphasia",
                        checkItems.aphasia ? "Present" : "Absent",
                        criteriaDescriptions.aphasia,
                    ],
                    [
                        "Neglect",
                        checkItems.neglect ? "Present" : "Absent",
                        criteriaDescriptions.neglect,
                    ],
                ];
                ...
                if (isVANPositive) {
                    ...
                    doc.setFontSize(12);
                    doc.text(
                        "VAN POSITIVE: High likelihood of Large Vessel Occlusion (LVO)",
                        20,
                        finalY + 7
                    );
                    ...
                    doc.setFontSize(10);
                    doc.text(
                        "Consider immediate vascular imaging and specialized intervention.",
                        20,
                        finalY + 14
                    );
                } else {
                    ...
                    doc.setFontSize(12);
                    doc.text(
                        "VAN NEGATIVE: Weakness present but no VAN criteria detected",
                        20,
                        finalY + 7
                    );
                }
